...
interface Foo {
    function addLiquidity(uint256 lpMinted, uint256 maxAmountTokens, address token) external payable returns (uint256 amount, uint256 amount2);
}
...
contract ExchangeHelperTest is Test {
    ...
    function setUp() public {
        deployed = HuffDeployer.config().deploy(
            "mocks/TestExchangeHelp"
        );
    ...
    */
    ...
    function calcLiquidity(uint256 total_liquidity, uint256 eth_reserve, uint256 token_reserve, uint256 maxTokens, uint256 value) internal  pure
    ...
    function testAddLiquidityFuzz(uint256 lpSupply, uint256 eth_reserve, uint256 tokens_reserve, uint256 maxTokens, uint256 value) public {
        ...
        (uint256 expectedMint, uint256 expectedTokens) = calcLiquidity(lpSupply, eth_reserve, tokens_reserve, maxTokens, value);   

        (uint256 mintLpAmount, uint256 tokens) = Foo(deployed).addLiquidity{value: value}(lpSupply, maxTokens, address(token));
